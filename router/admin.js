...
router.post("/create-cate",upload.single("image"),async(req,res)=>{
    const{category,description}=req.body;
    const imagepath=`/uploads/${req.file.filename}`;

    try{
    const savecate=new create({
        category,description,image:imagepath
    })
    await savecate.save();
    const {email,name}=req.session.adminemail;
  
   return res.render('category',{success:"create category successfull",email,name});
}
catch(err){
    console.log(err.message);
}
...
router.post('/updateimg/:id', upload.single("image"),async(req,res)=>{
    try {
        const imagepath = `/uploads/${req.file.filename}`;
...
router.post('/add-product',upload.array("image1",10),async(req,res)=>{
    const{category,subcategory,product,description, price,discountPrice,stock,stockStatus}=req.body;
     const imagepaths=req.files.map((file) => `/uploads/${file.filename}`);
    try{
        const product1= new addproduct({
            category,subcategory,product,description, price,discountPrice,stock,stockStatus,image1:imagepaths
        })
...
router.post('/collproduct',upload.array("image2",10),async(req,res)=>{
    ...
    try{
        ...
        image1:imagepath,product,price,stockStatus,discountPrice,description
        })
...
router.post('/editproduct/:id',async(req,res)=>{
    const{product,category,subcategory,discountPrice,description,price}=req.body;
    await addproduct.findByIdAndUpdate(req.params.id,{product,category,subcategory,discountPrice,description,price});
